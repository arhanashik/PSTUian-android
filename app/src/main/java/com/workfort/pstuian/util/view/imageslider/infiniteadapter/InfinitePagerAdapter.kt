package com.workfort.pstuian.util.view.imageslider.infiniteadapter
...
/**
 *  ****************************************************************************
 *  * Created by : arhan on 27 Nov, 2021 at 6:09.
...
class InfinitePagerAdapter(private val adapter: SliderViewAdapter<*>) : PagerAdapter() {
    ...
    fun getRealAdapter(): PagerAdapter {
        return adapter
    }

    override fun getCount(): Int {
        return if (getRealCount() < 1) 0 else getRealCount() * INFINITE_SCROLL_LIMIT
    ...
    fun getMiddlePosition(item: Int): Int {
        val midpoint = max(0, getRealCount()) * (INFINITE_SCROLL_LIMIT / 2)
