...
/**
 *  ****************************************************************************
 *  * Created by : arhan on 27 Nov, 2021 at 8:46.
...
class DropAnimation(listener: UpdateListener) : BaseAnimation<AnimatorSet?>(listener) {
    ...
    override fun progress(progress: Float): DropAnimation {
        if (animator != null) {
            ...
            for (anim in animator!!.childAnimations) {
                ...
                if (animator.values != null && animator.values.isNotEmpty()) {
    ...
    private fun createValueAnimation(
        fromValue: Int,
        toValue: Int,
        duration: Long,
        type: AnimationType
