...
internal class StudentProfileViewModel(
    ...
    fun onClickEditBio() {
        if (profileCache()?.isSignedIn != true) return
        profileCache()?.student?.let { student ->
            uiStateMachine.showMessage(MessageState.InputBio(student.bio.orEmpty()))
        }
    }
    ...
    fun onClickEdit(selectedTabIndex: Int) {
        ...
        profileCache()?.student?.let { student ->
            when (selectedTabIndex) {
                ...
                uiStateMachine.navigateTo(
                    NavigationState.StudentProfileEditScreen(
                        userId = student.id,
                        action = action,
                    )
                )
    ...
    fun onClickMyBloodDonationList() {
        ...
        profileCache()?.student?.let { student ->
            uiStateMachine.navigateTo(
                NavigationState.MyBloodDonationListScreen(
                    userId = student.id,
                    userType = UserType.STUDENT,
                )
            )
    ...
    fun onClickDownloadCv(url: String) {
        profileCache()?.student?.let { student ->
            uiStateMachine.navigateTo(
                NavigationState.DownloadCvScreen(
                    userId = student.id,
                    userType = UserType.STUDENT,
                    url = url,
                )
    ...
    fun onClickUploadCv() {
        ...
        profileCache()?.student?.let { student ->
            uiStateMachine.navigateTo(
                NavigationState.UploadCvScreen(
                    userId = student.id,
                    userType = UserType.STUDENT,
                )
            )
    ...
    fun onClickMyCheckInList() {
        ...
        profileCache()?.student?.let { student ->
            uiStateMachine.navigateTo(
                NavigationState.MyCheckInListScreen(
                    userId = student.id,
                    userType = UserType.STUDENT,
                )
            )
