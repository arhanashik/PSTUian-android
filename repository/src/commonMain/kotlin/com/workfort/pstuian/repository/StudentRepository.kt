...
class StudentRepository(
    ...
    suspend fun getProfile(studentId: Int): StudentProfile {
        // get student
        var student = studentDbService.get(studentId)?.toDto()
        if (student == null) {
            student = helper.get(studentId)
            studentDbService.insert(student.toEntity())
        }
        // get faculty
        val faculty = facultyRepo.getFaculty(student.facultyId).toDto()
        // get batch
        val batch = facultyRepo.getBatch(student.batchId).toDto()
        // get sign in state
        val isSignedIn = try {
